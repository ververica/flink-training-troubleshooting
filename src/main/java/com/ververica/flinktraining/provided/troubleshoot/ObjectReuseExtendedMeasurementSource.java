...
@DoNotChangeThis
public class ObjectReuseExtendedMeasurementSource extends RichParallelSourceFunction<ExtendedMeasurement> {
    ...
    private void initSensors() {
        final SplittableRandom rnd = new SplittableRandom();
        final ExtendedMeasurement.SensorType[] sensorTypes =
                ExtendedMeasurement.SensorType.values();
     ...
     */
    private static ExtendedMeasurement.Location randomInitialLocation(SplittableRandom rnd, int i) {
        final double longitude;
        final double latitude;
        // let's assume that no selected region wraps around LON -180/+180
        if (i < NUM_SENSORS / 2) {
            // in US
            longitude = rnd.nextDouble() * (GeoUtils.US_LON_EAST - GeoUtils.US_LON_WEST) + GeoUtils.US_LON_WEST;
            latitude = rnd.nextDouble() * (GeoUtils.US_LAT_NORTH - GeoUtils.US_LAT_SOUTH) + GeoUtils.US_LAT_SOUTH;
        } else {
            // in DE
            longitude = rnd.nextDouble() * (GeoUtils.DE_LON_EAST - GeoUtils.DE_LON_WEST) + GeoUtils.DE_LON_WEST;
            latitude = rnd.nextDouble() * (GeoUtils.DE_LAT_NORTH - GeoUtils.DE_LAT_SOUTH) + GeoUtils.DE_LAT_SOUTH;
        }
